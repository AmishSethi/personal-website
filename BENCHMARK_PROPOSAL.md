...
Status: v0.5 proposal · June 2026 · two rounds of three-track adversarial review complete (eval methodology, frontier-lab adoption, SRE domain + prior-art audit); round-2 verdicts: WOULD ADOPT / MINOR REVISIONS / MINOR REVISIONS, with all requested revisions incorporated in this version
...
- Simulated time advances from three sources only: (a) **tool invocations**, charged from a pinned public cost table reflecting realistic execution durations (a `kubectl get pods` costs seconds; a database restore costs its realistic duration); (b) **generation**, charged at a published exchange rate per generated *character* — tokenizer-neutral, so no model is scored on its vocabulary — meaning thinking is not free and the agent cannot stop the world to deliberate; (c) an explicit **`wait(t)`** primitive for deliberate watching.
- **Long-running tools can be launched in the background** with poll/wait semantics — start the 4-hour restore, tail logs while it runs — so the agent is not structurally single-threaded during long operations. The same primitive is exposed to human-baseline operators through the identical interface.
...
- Wall-clock is fully decoupled: host CPU contention and model latency cannot leak into scores (the Terminal-Bench ±6pt lesson), while time pressure — the core construct of on-call — is preserved through the generation charge.
- **Pre-registered ablation (Phase 2): rankings must be stable under ±2× perturbation of both the tool-cost table and the character exchange rate.** If they are not, EBR differences are artifacts of the time economy and the design returns to the shop. A secondary wall-clock-coupled variant is reported for transparency.
- The human-baseline condition charges sim-time from wall-clock seconds at a separately calibrated exchange rate (humans cannot ingest telemetry at generation speed; §4.4), with pause protocols for multi-hour shifts. **The calibration procedure is itself pre-registered** alongside the tick-cost table (matched telemetry-reading and remediation-throughput tasks performed by pilot SREs), because the human-vs-agent comparison is only as honest as this rate.
- Named engineering tension, stated rather than hidden: closed-loop cascade physics (§3.1) emerge from real processes with real timers, while sim-time decoupling requires the world to advance coherently between agent turns. The execution model (time-dilated slices vs. checkpoint/advance) is Phase 1's core deliverable; its correctness is gated by the replay-determinism study (§5), which tests the outcome rather than trusting the mechanism.
...
- **Aggregation (one formula, used everywhere):** episode EBRs → mean per (world, schedule) cell → equal-weighted mean across cells = **the model's EBR, and the headline number**. There is no second headline formula. Budget-weighted aggregation is also published as a robustness view; the pre-registration commits to both before any model is scored.

### 4.2 Headline and the reliability gate

- **Headline: one number.** The cell-aggregated EBR of §4.1, branded "OnCallBench: NN". No compound notation on any surface above the methods section.
- **Reliability is a gate, not a co-headline.** A leaderboard score is marked **Verified-Reliable** only if, on the *same* full-grid run that produces the headline (§6): (a) the P10 of the pooled episode-level EBR distribution satisfies **P10 ≥ max(0, 0.5 × headline EBR)** — the absolute floor prevents the gate from going vacuous at small positive headlines — and (b) Harm Rate ≤ 5%. **Both gate decisions are CI-aware, not point estimates**: the bootstrap lower bound of P10 and the upper 90% binomial bound of Harm Rate are what get compared to the thresholds, so the gate cannot flip on one or two lucky or unlucky episodes. Labs never have to print their worst number — the steward's gate does the work pass@1 marketing has dodged since τ-bench. (v0.3's median/min-of-5 co-headline is gone: review showed min-of-5 is an extreme order statistic that flips rankings by luck, and labs would simply omit it.)
- **Clean/contested stratification, stated at the schedule level** (scores are schedule-integrals and cannot be attributed per-template): **headline schedules are composed exclusively of oracle-clean templates**; any schedule containing a contested template belongs to the contested column. Contested schedules are scored against a published *strategy frontier* (each acceptable strategy — rollback, fix-forward, shed-and-scale — gets its own calibrated trajectory), with the agent credited via a **mechanical matching rule**: trajectories are classified by their irreversible-action signature (which irreversible actions were taken, in what order), pre-registered before the contested column ships — no discretionary matcher, because a discretionary matcher is a judge re-entering through the side door.
- **Statistical discipline:** bootstrap CIs accompany every published delta; rank claims are made only where CIs separate.
...
- **Full eval:** 6 worlds × 3 admitted schedules × 10 seeds = **180 episodes**, parallelized, <48h wall-clock. All 18 cells are oracle-clean (headline) schedules — the gate pool and the headline pool are the same 180 episodes; contested-column schedules run as a separate, additional set. Cost: provisionally $8–25K/model all-in (review judged v0.3's $3–8K optimistic by 2–3× given telemetry-dense contexts; the pilot publishes token actuals and this number gets replaced by measurement).
- **Official fast split (new — the RL-checkpoint loop requirement):** 2 public worlds × 1 schedule × 3 seeds, compressed shifts, ~4 hours end-to-end, **with a pilot-validated ≥0.9 Spearman correlation to the full score as a release criterion — validated across ≥20 runs spanning model checkpoints and scaffold variants**, not across the 5 frontier models alone (rank correlation at n=5 is meaningless and would be a fake gate). Benchmarks that can't sit inside a training loop get run once per launch and stop being news; the fast split is built deliberately, not discovered accidentally.
...
| 2. Frontier pilot | 8 wks | 5 frontier models + both human-baseline conditions; **pre-registered**: variance decomposition (world/schedule/seed/model-side), time-economy ±2× ablation (tool table + character rate), power analysis that *sets* the final seed/schedule budget, test-retest reliability, oracle-vs-human validation, human exchange-rate calibration, compute-cost actuals, fast-split correlation (≥20 checkpoint/scaffold runs) |
...
3. **Steward economics, costed rather than assumed.** Year-2 operations — quarterly re-runs across a growing roster at $8–25K/model, human-baseline refreshes, oracle recalibration, container upkeep — run an estimated **$0.75–1.5M/yr**. Funding sources, with the boundary drawn explicitly so Phase 0 negotiations don't stall on it: philanthropic/science funders and **non-lab commercial licensing** (enterprises licensing the harness and dev worlds to evaluate their own internal agents — a real revenue line that never touches the private split). Frontier labs may pay a **publicly priced, identical-for-all evaluation fee** for leaderboard runs; they may not fund private-split construction or steward general operations. "Lab pays a posted fee for a service" is auditable; "lab funds the steward" is FrontierMath.
4. **Comms risk handled ex ante:** all five frontier launch scores publish simultaneously (no one is singled out); trajectory publication for current packs follows a disclosure policy agreed with participating labs before the pilot (catastrophic-episode anecdotes — "the model bricked production" — are exactly the screenshot risk that would otherwise make every lab decline); the steward, not labs and not us, controls release timing. **The same ex-ante policy explicitly covers retired-pack disclosure**: catastrophic trajectories release with standardized context (harness version, oracle gap on that schedule, model generation and its current status), agreed before the pilot — labs price in the quarter-delayed story with known framing rules instead of fearing an unbounded one.
5. **Safety note on the fault library:** generic fault injection adds nothing beyond Chaos Mesh/Gremlin and public postmortems, and the taxonomy is published openly. The one artifact with modest sabotage-uplift value — the curated trap-dynamics + oracle-timeline corpus ("which remediations convert small outages into catastrophes") — stays in the private split as a safety measure as well as an anti-contamination one, with a responsible-release note in the paper.
...
**Round 2 (v0.4 → v0.5).** The same three reviewer tracks re-reviewed v0.4 against their round-1 blocking objections. Verdicts: eval-methodology **MINOR REVISIONS** (all 8 blockers confirmed addressed; 6 spec-tightening items), frontier-lab adoption **WOULD ADOPT** (all blockers addressed; would cite the steward-run leaderboard in a flagship announcement if launched as specified), SRE domain **MINOR REVISIONS** (all 6 blockers addressed; "would genuinely measure the core of on-call competence"). v0.5 incorporates every round-2 item: single headline formula (equal-weighted cell mean — the §4.1/§4.2 median/mean inconsistency removed), CI-aware reliability gate with absolute floor (P10 ≥ max(0, 0.5×headline) on bootstrap bounds; Harm Rate on binomial upper bound; computed on the same full-grid run as the headline), schedule-level clean/contested stratification with a mechanical irreversible-action-signature matching rule, tokenizer-neutral per-character generation charge with the exchange rate inside the ±2× ablation, background-execution primitive for long-running tools (agent and human conditions alike), fast-split correlation validated across ≥20 checkpoint/scaffold runs, winsorization-point justification, named time-virtualization engineering tension gated by the replay study, steward operating economics with an explicit lab-funding boundary (posted identical evaluation fees allowed; funding the steward or private split not), and retired-pack catastrophic-trajectory disclosure norms agreed ex ante.

**Round 1 (v0.3 → v0.4).** v0.4 incorporated three independent adversarial reviews of v0.3: (1) **eval-methodology** (verdict: major revisions — led to: unclipped EBR + Harm Rate, normative time economy + tick ablation, probe-defense section, schedule-level oracle, denominator admission rule, P10@k=10 reliability gate replacing min-of-5, human time-parity, oracle versioning replacing the "comparable by construction" claim, replay-determinism gate, restore-primitive pricing, toil physics, multi-key root-cause taxonomy, cost restatement); (2) **frontier-lab adoption** (verdict: would adopt with changes — led to: steward-as-precondition, single-number headline + steward-enforced gate, simultaneous five-model launch + disclosure policy, pre-registered power/test-retest requirements, retired-pack disclosure + adjudication channel, official fast split, pinned reference scaffold, year-one/year-two framing); (3) **SRE domain + prior-art audit** (verdict: major revisions — led to: the rename, the corrected prior-art table incl. ITBench-AA/Cloud-OpsBench/DevOps-Gym/SREBench/SRE-skills-bench/OpenRCA, the re-sequenced steward strategy, closed-loop traffic, alert fan-out, observability-gap faults, cold/warm human baselines, contested-oracle strategy frontier, contamination-boundary framing, trap-library gating).
